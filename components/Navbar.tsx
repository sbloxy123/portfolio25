...
const Navbar = () => {
  ...
  return (
    ...
    >
      <div
        className={`relative transition-all duration-300 ${toggleMenu ? 'z-20 translate-y-[-110%] opacity-0 xsmall:translate-y-0 small:opacity-100' : 'opacity-100'}`}
      >
        <Link
          href="/"
      ...
      <div
        className={`icon__links absolute bottom-0 left-[5%] right-0 top-0 z-20 m-auto ml-0 hidden h-fit w-fit transition-opacity duration-500 xsmall:left-0 xsmall:ml-auto tablet:block ${
      ...
      <div className="menu__container inset-0 z-10">
        ...
        >
          <div
            className={`menu__icon__links absolute bottom-auto left-[5%] right-auto top-8 z-20 m-auto ml-auto h-fit w-[50vw] max-w-[700px] translate-y-2 transition-opacity duration-1000 xsmall:top-12 small:top-[3rem] ${toggleMenu ? 'opacity-100' : 'opacity-0'}`}
          >
